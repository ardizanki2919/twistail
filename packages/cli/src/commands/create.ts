...
export default defineCommand({
  meta: {
    name: 'create',
    description: 'Create new React application from a template',
  },
  args: {
    name: {
      type: 'positional',
  ...
  async setup({ args }) {
    ...
    try {
      ...
      if (await checkDirectoryExists('twistail', 'templates', template)) {
        if (!silent) {
  ...
  async run({ args }) {
    ...
    try {
      ...
      const templateUrl = `github:riipandi/twistail/${template}`
